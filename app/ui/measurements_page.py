"""PulseView — Premium measurements entry & history."""
...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QScrollArea, QSizePolicy, QSpinBox, QTableWidget, QTableWidgetItem,
...
class MeasurementsPage(QWidget):
    ...
    def _build(self):
        ...
        layout.addWidget(PageHeader("Вимірювання", "Внесення нових записів та перегляд історії"))
        ...
        self.datetime_edit.setStyleSheet("QDateTimeEdit{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,stop:0 #fafbff,stop:1 #f5f7ff);border:1.5px solid #e2e8f0;border-radius:12px;padding:8px 14px;color:#1e293b;font-size:13px;min-height:34px;}")

        self.systolic_spin = QSpinBox(); self.systolic_spin.setRange(60, 240); self.systolic_spin.setValue(120); self.systolic_spin.setButtonSymbols(QSpinBox.ButtonSymbols.NoButtons)
        self.diastolic_spin = QSpinBox(); self.diastolic_spin.setRange(40, 150); self.diastolic_spin.setValue(80); self.diastolic_spin.setButtonSymbols(QSpinBox.ButtonSymbols.NoButtons)
        self.pulse_spin = QSpinBox(); self.pulse_spin.setRange(35, 220); self.pulse_spin.setValue(72); self.pulse_spin.setButtonSymbols(QSpinBox.ButtonSymbols.NoButtons)
        self.atm_spin = QSpinBox(); self.atm_spin.setRange(680, 810); self.atm_spin.setValue(745); self.atm_spin.setButtonSymbols(QSpinBox.ButtonSymbols.NoButtons)

        self.city_box = QComboBox(); self.city_box.addItems(city_names())
        self.geo_btn = QPushButton("📍")
        self.geo_btn.setFixedWidth(60)
        self.geo_btn.setToolTip("Визначити місто за вашим місцезнаходженням (через інтернет / IP)")
        ...
        self.mood_box.addItems(["Спокійний", "Робочий день", "Стрес", "Після тренування", "Незадовільне самопочуття"])
        ...
        self.notes_edit = QTextEdit(); self.notes_edit.setFixedHeight(90)
        self.notes_edit.setPlaceholderText("Коротка примітка про стан, час доби, фізичну активність тощо")
        ...
        city_row = QHBoxLayout(); city_row.addWidget(self.city_box, 1); city_row.addWidget(self.geo_btn)
        form.addRow("Місто:", city_row)
        QTimer.singleShot(400, lambda: self._detect_my_city(silent=True))
        atm_row = QHBoxLayout(); atm_row.addWidget(self.atm_spin); atm_row.addWidget(self.fetch_atm_btn)
        ...
        buttons.addWidget(self.add_btn); buttons.addWidget(self.clear_btn)
        ...
        hdr.setMinimumSectionSize(70); hdr.setDefaultSectionSize(100)
        ...
        self.prev_btn.setObjectName("secondaryButton"); self.prev_btn.setFixedWidth(90)
        self.prev_btn.clicked.connect(self._prev_page)
        self.next_btn = QPushButton("Далі")
        self.next_btn.setObjectName("secondaryButton"); self.next_btn.setFixedWidth(90)
        self.next_btn.clicked.connect(self._next_page)
        self.page_label = QLabel("Сторінка 1")
        self.page_label.setStyleSheet("color:#6366f1; font-size:12px; font-weight:700; letter-spacing:0.2px;")
        ...
        table_buttons.addWidget(self.prev_btn); table_buttons.addWidget(self.page_label)
        table_buttons.addWidget(self.next_btn); table_buttons.addStretch(1)
        table_buttons.addWidget(self.expand_btn); table_buttons.addWidget(self.delete_btn)
        table_layout.addLayout(table_buttons)

        top.addWidget(form_panel, 1); top.addWidget(table_panel, 2)
    ...
    def _detect_my_city(self, silent: bool = False) -> None:
        ...
        if not city:
            if not silent:
                QMessageBox.warning(self, "Геолокація", "Не вдалося визначити місто.\nПеревірте інтернет або оберіть місто вручну зі списку.")
            return
        ...
        if not silent:
            QMessageBox.information(self, "Геолокація", f"Обрано найближче місто зі списку: {city}.\nТочність залежить від провайдера (визначення за IP, не GPS).")

    def _fetch_atmospheric(self) -> None:
        city = self.city_box.currentText()
        value = fetch_atmospheric_pressure_mmhg(city)
        if value is None:
            QMessageBox.warning(self, "Погода", "Не вдалося отримати дані. Перевірте інтернет або введіть тиск вручну.")
            return
        self.atm_spin.setValue(value)
        QMessageBox.information(self, "Погода", f"Атмосферний тиск для {city}: {value} мм рт. ст.")

    def _clear_form(self) -> None:
        self.datetime_edit.setDateTime(QDateTime.currentDateTime())
        self.systolic_spin.setValue(120); self.diastolic_spin.setValue(80); self.pulse_spin.setValue(72)
        self.atm_spin.setValue(745); self.mood_box.setCurrentIndex(0); self.activity_box.setCurrentIndex(0)
        self.medication_check.setChecked(False); self.notes_edit.clear()

    def _add_measurement(self) -> None:
        measurement = Measurement(
            id=uuid.uuid4().hex[:8],
            timestamp=self.datetime_edit.dateTime().toString("yyyy-MM-dd HH:mm"),
            systolic=self.systolic_spin.value(), diastolic=self.diastolic_spin.value(),
            pulse=self.pulse_spin.value(), mood=self.mood_box.currentText(),
    ...
    def _delete_selected(self) -> None:
        ...
        answer = QMessageBox.question(self, "Підтвердження", f"Видалити запис від {measurement.timestamp}?")
        if answer == QMessageBox.StandardButton.Yes:
            self.delete_callback(measurement.id)

    def _prev_page(self) -> None:
        if self._current_page > 0:
            self._current_page -= 1; self._render_page()
    ...
    def _render_page(self) -> None:
        ...
        self.page_label.setText(f"Сторінка {self._current_page + 1} / {total_pages}  ({total} записів)")
        ...
        for row_index, measurement in enumerate(self.measurements):
            ...
            for col_index, cell_value in enumerate(measurement_to_row(measurement)):
                ...
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter if col_index < 7 else Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
                item.setBackground(row_bg)
                if col_index == 1:
                    try:
                        sys_val = int(cell_value.split("/")[0])
                        if sys_val >= 140:
                            item.setForeground(QColor("#e11d48")); f = QFont(); f.setBold(True); item.setFont(f)
                        elif sys_val < 100:
    ...
    def _open_fullscreen_table(self) -> None:
        ...
        dlg.setMinimumSize(1100, 680); dlg.resize(1280, 720)
        vl = QVBoxLayout(dlg); vl.setContentsMargins(24, 24, 24, 24); vl.setSpacing(16)
        hdr_row = QHBoxLayout()
        ...
        hdr_row.addWidget(title); hdr_row.addStretch(1); hdr_row.addWidget(close_btn)
        vl.addLayout(hdr_row)

        big_table = QTableWidget(0, 8)
        big_table.setHorizontalHeaderLabels(["Дата", "Тиск", "Пульс", "Атм.", "Стан", "Ліки", "Активність", "Примітки"])
        ...
        big_table.setAlternatingRowColors(True); big_table.setShowGrid(False)
        ...
        big_hdr.setDefaultSectionSize(120); big_table.verticalHeader().setDefaultSectionSize(42)
